...
func (checkQuotaCmd *CheckQuotaCommand) RunCommand(cmd *cobra.Command, args []string) error {
	// get fs id
	fsId, idErr := rpc.GetFsId(cmd)
	if idErr != nil {
		return idErr
	}
	// get filesystem name
	fsName, nameErr := rpc.GetFsName(cmd)
	if nameErr != nil {
		return nameErr
	}
	...
	realUsedBytes, realUsedInodes, getErr := rpc.GetDirectorySizeAndInodes(checkQuotaCmd.Cmd, fsId, config.ROOTINODEID, true, epoch, threads)
	if getErr != nil {
		return getErr
	}

	checkResult, ok := common.CheckQuota(fsQuota.GetMaxBytes(), fsQuota.GetUsedBytes(), fsQuota.GetMaxInodes(), fsQuota.GetUsedInodes(), realUsedBytes, realUsedInodes)
