...
func (setQuotaCmd *SetQuotaCommand) Init(cmd *cobra.Command, args []string) error {
	// check flags values
	maxBytes, maxInodes, quotaErr := common.CheckAndGetQuotaValue(setQuotaCmd.Cmd)
	if quotaErr != nil {
		return quotaErr
	}
	...
	//get inodeid
	dirInodeId, inodeErr := rpc.GetDirPathInodeId(setQuotaCmd.Cmd, fsId, path, epoch)
	if inodeErr != nil {
		return inodeErr
	}
	...
	realUsedBytes, realUsedInodes, getErr := rpc.GetDirectorySizeAndInodes(setQuotaCmd.Cmd, fsId, dirInodeId, false, epoch, threads)
	if getErr != nil {
		return getErr
	}
