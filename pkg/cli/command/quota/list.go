...
func (listQuotaCmd *ListQuotaCommand) RunCommand(cmd *cobra.Command, args []string) error {
	...
	for dirInode, quota := range dirQuotas {
		...
		quotaValueSlice := common.ConvertQuotaToHumanizeValue(uint64(quota.GetMaxBytes()), quota.GetUsedBytes(), uint64(quota.GetMaxInodes()), quota.GetUsedInodes())

		dirPath, _, dirErr := rpc.GetInodePath(listQuotaCmd.Cmd, listQuotaCmd.fsId, dirInode, listQuotaCmd.epoch)
