...
func (queryFSCmd *QueryFsCommand) Init(cmd *cobra.Command, args []string) error {
	...
	for _, fsName := range fsNames {
		...
		rpc.Info = mdsRpc
		queryFSCmd.Rpc = append(queryFSCmd.Rpc, rpc)
	}
	// query by fsids
	...
	// set table header
	header := []string{cobrautil.ROW_FS_ID, cobrautil.ROW_FS_NAME, cobrautil.ROW_STATUS, cobrautil.ROW_BLOCKSIZE, cobrautil.ROW_CHUNK_SIZE, cobrautil.ROW_STORAGE_TYPE, cobrautil.ROW_OWNER, cobrautil.ROW_MOUNT_NUM, cobrautil.ROW_UUID}
	...
	queryFSCmd.TableNew.SetAutoMergeCellsByColumnIndex([]int{indexOwner, indexType})
...
func (queryFSCmd *QueryFsCommand) RunCommand(cmd *cobra.Command, args []string) error {
	...
	responses, errs := basecmd.GetRpcListResponse(infos, funcs)
