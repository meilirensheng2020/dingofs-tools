...
func (registerMember *RegisterMemberCommand) RunCommand(cmd *cobra.Command, args []string) error {
	// new rpc
	mdsRpc, err := common.CreateNewMdsRpc(cmd, "RegisterMember")
	if err != nil {
		return err
	}
	...
	registerMember.Rpc = &base.RegisterCacheMemberRpc{
		Info: mdsRpc,
	...
	response, cmdErr := base.GetRpcResponse(registerMember.Rpc.Info, registerMember.Rpc)
