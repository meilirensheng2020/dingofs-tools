...
func (mdsCmd *MdsCommand) Init(cmd *cobra.Command, args []string) error {
	...
	// set request info
	mdsCmd.Rpc = &common.GetMdsRpc{Info: mdsRpc, Request: &pbmdsv2.GetMDSListRequest{}}
	// set table header
...
func (mdsCmd *MdsCommand) RunCommand(cmd *cobra.Command, args []string) error {
	// get rpc result
	response, errCmd := basecmd.GetRpcResponse(mdsCmd.Rpc.Info, mdsCmd.Rpc)
