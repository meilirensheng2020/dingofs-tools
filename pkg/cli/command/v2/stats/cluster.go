...
func (clusterCmd *ClusterCommand) Init(cmd *cobra.Command, args []string) error {
	...
	// set request info
	fsName, fsErr := common.GetFsName(clusterCmd.Cmd)
	if fsErr != nil {
		return fsErr
	}
	clusterCmd.Rpc = &common.GetFsStatsRpc{
		Info: mdsRpc,
...
func (clusterCmd *ClusterCommand) GetFsStatsData() (map[string]float64, error) {
	// get rpc result
	response, errCmd := basecmd.GetRpcResponse(clusterCmd.Rpc.Info, clusterCmd.Rpc)
