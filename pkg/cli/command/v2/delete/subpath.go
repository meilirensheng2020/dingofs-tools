...
func (subPathCmd *SubPathCommand) Init(cmd *cobra.Command, args []string) error {
	...
	parentInodeId, inodeErr := common.GetDirPathInodeId(subPathCmd.Cmd, fsId, parentDirName, epoch)
	if inodeErr != nil {
		return inodeErr
	}
...
func (subPathCmd *SubPathCommand) CheckPathIsExist(cmd *cobra.Command) (bool, uint64) {
	entries, entErr := common.ListDentry(cmd, subPathCmd.fsId, subPathCmd.parentInodeId, subPathCmd.epoch)
	if entErr != nil {
		return false, 0
	}
...
func (subPathCmd *SubPathCommand) DeleteDirectoryAndData(cmd *cobra.Command, fsId uint32, parentInodeId uint64, dirInodeId uint64, name string, summary *common.Summary, concurrent chan struct{},
	...
	var err error
	entries, entErr := common.ListDentry(cmd, fsId, dirInodeId, subPathCmd.epoch)
	if entErr != nil {
		return entErr
	}
	...
	for _, entry := range entries {
		if entry.GetType() != pbmdsv2.FileType_DIRECTORY {
			err := common.DeleteFile(cmd, fsId, entry.GetParent(), entry.GetName(), subPathCmd.epoch)
			if err != nil {
				return err
			}
	...
	default:
		// delete self
		err := common.DeleteDirectory(cmd, fsId, parentInodeId, name, subPathCmd.epoch)
		if err != nil {
			return err
		}
