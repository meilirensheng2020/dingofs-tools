...
func (subPathCmd *SubPathCommand) Init(cmd *cobra.Command, args []string) error {
	...
	// get parent
	parentInodeId, inodeErr := common.GetDirPathInodeId(subPathCmd.Cmd, fsId, parentPathName, epoch)
	if inodeErr != nil {
		return inodeErr
	}
...
func (subPathCmd *SubPathCommand) CheckPathIsExist(cmd *cobra.Command) bool {
	entries, entErr := common.ListDentry(cmd, subPathCmd.inodeParam.fsId, subPathCmd.inodeParam.parent, subPathCmd.inodeParam.epoch)
	if entErr != nil {
		return false
	}
...
func (subPathCmd *SubPathCommand) MkDir(cmd *cobra.Command, inodeParam InodeParam) *cmderror.CmdError {
	// new prc request
	endpoint := common.GetEndPoint(inodeParam.parent)
	mdsRpc := common.CreateNewMdsRpcWithEndPoint(cmd, endpoint, "MkDir")

	mkDirRpc := &common.MkDirRpc{
		Info: mdsRpc,
