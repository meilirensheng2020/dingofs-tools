...
func (fCmd *FsCommand) RunCommand(cmd *cobra.Command, args []string) error {
	...
	if mdsErr.GetErrcode() == pbmdsv2error.Errno_OK {
		header := []string{cobrautil.ROW_FS_ID, cobrautil.ROW_FS_NAME, cobrautil.ROW_STATUS, cobrautil.ROW_BLOCKSIZE, cobrautil.ROW_CHUNK_SIZE, cobrautil.ROW_STORAGE_TYPE, cobrautil.ROW_OWNER, cobrautil.ROW_UUID, cobrautil.ROW_RESULT}
