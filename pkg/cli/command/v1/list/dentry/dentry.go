...
func (dentryCommand *DentryCommand) AddFlags() {
	...
	config.AddFsIdRequiredFlag(dentryCommand.Cmd)
...
func (dentryCommand *DentryCommand) RunCommand(cmd *cobra.Command, args []string) error {
	...
	entries, entErr := cmdcommon.ListDentry(cmd, fsId, inodeId)
	if entErr != nil {
		return entErr
	}
	...
	for _, entry := range entries {
		...
		row[cobrautil.ROW_INODE_ID] = fmt.Sprintf("%d", entry.GetInodeId())
		row[cobrautil.ROW_NAME] = entry.GetName()
		row[cobrautil.ROW_PARENT] = fmt.Sprintf("%d", entry.GetParentInodeId())
