...
func (checkQuotaCmd *CheckQuotaCommand) RunCommand(cmd *cobra.Command, args []string) error {
	...
	path := config.GetFlagString(checkQuotaCmd.Cmd, config.DINGOFS_QUOTA_PATH)
	...
	if repair && !ok { // inconsistent and need to repair
		...
		checkQuotaCmd.Rpc.Info = base.NewRpc(addrs, timeout, retrytimes, retryDelay, verbose, "SetDirQuota")
