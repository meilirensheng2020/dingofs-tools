...
func (fsUsageCmd *FsUageCommand) RunCommand(cmd *cobra.Command, args []string) error {
	...
	if !fsUsageCmd.Cmd.Flag(config.DINGOFS_FSID).Changed && !fsUsageCmd.Cmd.Flag(config.DINGOFS_FSNAME).Changed {
		...
		// one filesystem
		fsId, err := rpc.GetFsId(fsUsageCmd.Cmd)
		if err != nil {
			errGetFsUsage = cmderror.ErrGetFsUsage()
			errGetFsUsage.Format(err.Error())
			return nil
		}
		fsName, err := rpc.GetFsName(fsUsageCmd.Cmd)
		if err != nil {
			errGetFsUsage = cmderror.ErrGetFsUsage()
			errGetFsUsage.Format(err.Error())
			return nil
	...
	for idx, fsId := range fsIds {
		...
		realUsedBytes, realUsedInodes, err := rpc.GetDirectorySizeAndInodes(fsUsageCmd.Cmd, fsId, config.ROOTINODEID, true, epochs[idx], threads)
		if err != nil {
			errGetFsUsage = cmderror.ErrGetFsUsage()
			errGetFsUsage.Format(err.Error())
			return nil
